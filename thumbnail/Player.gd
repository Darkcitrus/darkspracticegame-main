...
@onready var attackcd = $sword/attackcd
...
func perform_sweep():
	...
	current_tween.tween_method(func(progress: float):
		...
		sword.global_position = global_position + sweep_direction * 50
...
func point_sword_to_mouse():
	if not attacking and current_tween == null:  # Only update if not attacking and not tweening
		...
		sword.global_position = global_position + direction_to_mouse * 50
